...
function isValidPlanPath(projectRoot: string, resolvedPath: string): boolean {
  ...
  if (
    resolvedPath.startsWith(mainPlansDir + path.sep) ||
    resolvedPath.startsWith(mainPlansDir + "/")
  ) {
    return true;
  }
  const worktreesDir = path.join(normalizedRoot, ".worktrees");
  if (
    resolvedPath.startsWith(worktreesDir) &&
    resolvedPath.includes("/docs/plans/")
  ) {
    return true;
  }
  return false;
...
export class PlanRoutes extends BaseRouteHandler {
  ...
  private static VALID_PLAN_STATUSES = new Set([
    "PENDING",
    "COMPLETE",
    "VERIFIED",
  ]);
  ...
  private isValidPlanStatus(status: unknown): status is PlanInfo["status"] {
    return (
      typeof status === "string" && PlanRoutes.VALID_PLAN_STATUSES.has(status)
    );
  }
  ...
  setupRoutes(app: express.Application): void {
    ...
    app.post(
      "/api/sessions/:sessionDbId/plan",
      this.handleAssociatePlan.bind(this),
    );
    app.post(
    ...
    app.get(
      "/api/sessions/:sessionDbId/plan",
      this.handleGetSessionPlan.bind(this),
    );
    app.get(
      "/api/sessions/by-content-id/:contentSessionId/plan",
    ...
    app.delete(
      "/api/sessions/:sessionDbId/plan",
      this.handleClearSessionPlan.bind(this),
    );
    app.put(
      "/api/sessions/:sessionDbId/plan/status",
      this.handleUpdatePlanStatus.bind(this),
    );
  }

  private handleGetPlanStats = this.wrapHandler(
  ...
  private handleGetActivePlan = this.wrapHandler(
    (req: Request, res: Response): void => {
      const project = req.query.project as string | undefined;
      const projectRoot = resolveProjectRoot(this.dbManager, project);
  ...
  private handleGetAllPlans = this.wrapHandler(
    (req: Request, res: Response): void => {
      const project = req.query.project as string | undefined;
      const projectRoot = resolveProjectRoot(this.dbManager, project);
  ...
  private handleGetGitInfo = this.wrapHandler(
    (req: Request, res: Response): void => {
      const project = req.query.project as string | undefined;
      const projectRoot = resolveProjectRoot(this.dbManager, project);
  ...
  private handleGetActiveSpecs = this.wrapHandler(
    (req: Request, res: Response): void => {
      const project = req.query.project as string | undefined;
      const projectRoot = resolveProjectRoot(this.dbManager, project);
  ...
  private handleGetPlanContent = this.wrapHandler(
    (req: Request, res: Response): void => {
      const project = req.query.project as string | undefined;
      const projectRoot = resolveProjectRoot(this.dbManager, project);
      ...
      if (!isValidPlanPath(projectRoot, resolvedPath)) {
        res
          ...
          .json({
            error:
              "Access denied: path must be within docs/plans/ or .worktrees/*/docs/plans/",
          });
        return;
      }
  ...
  private handleDeletePlan = this.wrapHandler(
    (req: Request, res: Response): void => {
      const project = req.query.project as string | undefined;
      const projectRoot = resolveProjectRoot(this.dbManager, project);
      const requestedPath = req.query.path as string | undefined;
  ...
  private handleAssociatePlan = this.wrapHandler(
    (req: Request, res: Response): void => {
      ...
      if (!this.isValidPlanStatus(req.body.status)) {
        this.badRequest(
          res,
          `Invalid status: ${req.body.status}. Must be PENDING, COMPLETE, or VERIFIED`,
        );
        return;
      }
  ...
  private handleAssociatePlanByContentId = this.wrapHandler(
    (req: Request, res: Response): void => {
  ...
  private handleGetSessionPlan = this.wrapHandler(
    (req: Request, res: Response): void => {
      const sessionDbId = this.parseIntParam(req, res, "sessionDbId");
      if (sessionDbId === null) return;
      const db = this.getDb(res);
  ...
  private handleGetSessionPlanByContentId = this.wrapHandler(
    (req: Request, res: Response): void => {
      const contentSessionId = req.params.contentSessionId;
      if (!contentSessionId) {
        this.badRequest(res, "Missing contentSessionId");
  ...
  private handleClearSessionPlan = this.wrapHandler(
    (req: Request, res: Response): void => {
      const sessionDbId = this.parseIntParam(req, res, "sessionDbId");
      if (sessionDbId === null) return;
      const db = this.getDb(res);
  ...
  private handleUpdatePlanStatus = this.wrapHandler(
    (req: Request, res: Response): void => {
      const sessionDbId = this.parseIntParam(req, res, "sessionDbId");
      if (sessionDbId === null) return;
